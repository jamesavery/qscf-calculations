...
./SET-mesh2.pl $molecule > $directory/${molecule}-set.geo
pushd $directory
...
for charge in -2.0 -1.0 0.0 1.0 2.0
  ...
  for ground in 8.0 -6.0 -4.0 -3.0  -2.0 -1.0  0.0  1.0  2.0 3.0 4.0 6.0 8.0
    do    
    ./SET-input2.pl $molecule $charge $V $ground $final_dE > ${directory}/SET.${charge}:${ground}:${V}.in
  done
done
...
for charge in -2.0 -1.0 0.0 1.0 2.0
  ...
  for ground in 8.0 6.0 -4.0 -3.0 -2.0 0.0 -1.0 1.0  2.0 3.0  4.0 6.0 8.0 
    do    
    ./SET-input2.pl $molecule $charge $V $ground $final_dE > ${directory}/SET-2.${charge}:${ground}:${V}.in
  done
done
